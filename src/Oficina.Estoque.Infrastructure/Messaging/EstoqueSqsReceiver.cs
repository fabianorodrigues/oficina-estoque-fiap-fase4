using Amazon.SQS;
using Amazon.SQS.Model;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Logging;
using Oficina.Estoque.Infrastructure.Persistencia;

namespace Oficina.Estoque.Infrastructure.Messaging;

internal sealed class EstoqueSqsReceiver(
    IAmazonSQS sqs,
    IServiceScopeFactory scopes,
    Microsoft.Extensions.Options.IOptions<SqsMessagingOptions> options,
    ILogger<EstoqueSqsReceiver> logger) : SqsBackgroundService(logger)
{
    private string? _queueUrl;

    protected override async Task ExecuteOnce(CancellationToken ct)
    {
        _queueUrl ??= await QueueUrlResolver.Resolve(sqs, options.Value, options.Value.CommandsQueueName, options.Value.CommandsQueueUrl, ct);
        var response = await sqs.ReceiveMessageAsync(new ReceiveMessageRequest
        {
            QueueUrl = _queueUrl,
            MaxNumberOfMessages = options.Value.MaxMessages,
            WaitTimeSeconds = options.Value.WaitTimeSeconds,
            VisibilityTimeout = options.Value.VisibilityTimeoutSeconds
        }, ct);

        foreach (var message in response.Messages ?? [])
        {
            MessageEnvelope envelope;
            try
            {
                envelope = MessageJson.ParseAndValidate(message.Body);
                if (envelope.MessageType is not (EstoqueMessageTypes.ReservarEstoque or EstoqueMessageTypes.LiberarReservaEstoque))
                    throw new InvalidOperationException("Comando desconhecido.");
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Mensagem invalida recebida antes da Inbox. Sem ACK para redrive nativo.");
                continue;
            }

            await using var scope = scopes.CreateAsyncScope();
            var db = scope.ServiceProvider.GetRequiredService<EstoqueDbContext>();
            if (!await db.InboxMessages.AnyAsync(x => x.MessageId == envelope.MessageId, ct))
            {
                db.InboxMessages.Add(new InboxMessage(envelope.MessageId, envelope.MessageType, envelope.OrdemServicoId, envelope.CorrelationId, message.Body));
                await db.SaveChangesAsync(ct);
            }

            await sqs.DeleteMessageAsync(_queueUrl, message.ReceiptHandle, ct);
        }
    }
}
