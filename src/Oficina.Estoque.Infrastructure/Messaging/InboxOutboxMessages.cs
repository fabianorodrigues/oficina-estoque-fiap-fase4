...
public sealed class InboxMessage
{
    ...
    public void Claim(DateTimeOffset lockedUntilUtc)
    {
    ...
    {
        Status = InboxMessageStatus.Deferred;
        LockedUntilUtc = DateTimeOffset.UtcNow.AddSeconds(30);
        Error = Sanitize(reason);
    }

    public void MarkFailed(string error, bool deadLetter)
    {
        Status = deadLetter ? InboxMessageStatus.DeadLettered : InboxMessageStatus.Received;
        LockedUntilUtc = null;
...
public sealed class OutboxMessage
{
    ...
    {
        LockedUntilUtc = null;
        Error = error.Length <= 500 ? error : error[..500];
    }
}
