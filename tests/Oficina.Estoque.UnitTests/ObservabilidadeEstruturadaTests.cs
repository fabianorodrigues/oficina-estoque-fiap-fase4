...
public class OficinaJsonConsoleFormatterTests
{
    ...
    public void Deve_emitir_os_campos_do_contrato_no_nivel_superior()
    {
        ...
        Assert.Equal("oficina-estoque", json.GetProperty("service.name").GetString());
    ...
    {
        using var source = new ActivitySource("Oficina.Estoque.Testes");
        using var listener = new ActivityListener
        {
            ShouldListenTo = candidate => candidate.Name == "Oficina.Estoque.Testes",
...
public class OficinaTelemetryResourceTests
{
    ...
    {
        var resource = OficinaTelemetryResource.Resolve(Configuracao(), "oficina-estoque");
...
public class CorrelationIdMiddlewareObservabilidadeTests
{
    ...
    {
        using var source = new ActivitySource("Oficina.Estoque.Testes.Middleware");
        using var listener = new ActivityListener
        {
            ShouldListenTo = candidate => candidate.Name == "Oficina.Estoque.Testes.Middleware",
            Sample = (ref ActivityCreationOptions<ActivityContext> _) => ActivitySamplingResult.AllDataAndRecorded
        };
        ActivitySource.AddActivityListener(listener);
